const fs = require('fs');
const path = require('path');

const BbPromise = require('bluebird');
const mime = require('mime');

const getFileList = require('./utilities/getFileList');

/**
 * Uploads client files to an S3 bucket
 * @param {Object} aws - AWS class
 * @param {string} bucketName - Name of bucket to be configured
 * @param {string} clientRoot - Full path to the root directory of client files
 * @param {Object[]} headerSpec - Array of header values to add to files
 * @param {Object} logger - Optional logger instance
 * @param {number} concurrency - Maximum number of parallel uploads (default: 250)
 */
function uploadDirectory(aws, bucketName, clientRoot, headerSpec, logger = null, concurrency = 250) {
    if (logger) {
        logger.startTimer('uploadDirectory');
        logger.info(`Scanning directory for files to upload: ${clientRoot}`);
    }
    
    const allFiles = getFileList(clientRoot);
    
    if (logger) {
        logger.verbose(`Found ${allFiles.length} file(s) in directory`);
    }

    const uploadList = buildUploadList(allFiles, clientRoot, headerSpec, logger);
    
    // Calculate total size before starting upload
    let totalSize = 0;
    const fileSizes = [];
    uploadList.forEach(u => {
        try {
            const stats = fs.statSync(u.filePath);
            const size = stats.size;
            totalSize += size;
            fileSizes.push({ key: u.fileKey, size });
        } catch (e) {
            // File might not exist, skip size calculation
        }
    });

    // Show summary BEFORE starting upload
    if (logger) {
        const sizeInMB = (totalSize / (1024 * 1024)).toFixed(2);
        const sizeInKB = (totalSize / 1024).toFixed(2);
        const sizeDisplay = totalSize > 1024 * 1024 ? `${sizeInMB} MB` : `${sizeInKB} KB`;
        
        logger.info(`Prepared ${uploadList.length} file(s) for upload (${sizeDisplay} total)`);
        logger.info(`Starting upload to bucket '${bucketName}'...`);
        
        if (logger.isVerbose() && uploadList.length > 0) {
            // Show first few files in verbose mode
            const previewCount = Math.min(5, uploadList.length);
            logger.verbose(`Sample files to upload:`);
            for (let i = 0; i < previewCount; i++) {
                const size = fileSizes[i] ? ` (${(fileSizes[i].size / 1024).toFixed(2)} KB)` : '';
                logger.verbose(`  - ${uploadList[i].fileKey}${size}`);
            }
            if (uploadList.length > previewCount) {
                logger.verbose(`  ... and ${uploadList.length - previewCount} more file(s)`);
            }
        }
    }

    let completed = 0;
    const total = uploadList.length;
    let lastProgressPercent = -1;
    let lastProgressTime = Date.now();
    const minProgressInterval = 500; // Minimum 500ms between progress updates (more frequent)
    
    // Ensure concurrency is a positive integer
    const uploadConcurrency = Math.max(1, Math.floor(concurrency || 250));

    // Show initial progress
    if (logger) {
        logger.uploadProgress(0, total, null, -1);
        logger.verbose(`Uploading with concurrency limit of ${uploadConcurrency} parallel uploads`);
    }

    // Upload files with progress tracking and concurrency control
    return BbPromise.map(
        uploadList,
        (u, index) => {
            return uploadFile(aws, bucketName, u.filePath, u.fileKey, u.headers, logger)
                .then(result => {
                    // Use a simple increment - JavaScript is single-threaded so this is safe
                    completed++;
                    if (logger) {
                        const now = Date.now();
                        // Always show progress, but throttle updates to avoid spam
                        // Show immediately on completion, or if enough time has passed
                        if (completed === total || (now - lastProgressTime) >= minProgressInterval) {
                            lastProgressPercent = logger.uploadProgress(completed, total, u.fileKey, lastProgressPercent);
                            lastProgressTime = now;
                        }
                    }
                    return result;
                });
        },
        { concurrency: uploadConcurrency }
    ).then(results => {
        if (logger) {
            const elapsed = logger.endTimer('uploadDirectory');
            const sizeInMB = (totalSize / (1024 * 1024)).toFixed(2);
            const sizeInKB = (totalSize / 1024).toFixed(2);
            const sizeDisplay = totalSize > 1024 * 1024 ? `${sizeInMB} MB` : `${sizeInKB} KB`;
            logger.success(`Uploaded ${total} file(s) (${sizeDisplay}) successfully in ${logger.formatTime(elapsed)}`);
        }
        return results;
    });
}

/**
 * Uploads a file to an S3 bucket
 * @param {Object} aws - AWS class
 * @param {string} bucketName - Name of bucket to be configured
 * @param {string} filePath - Full path to file to be uploaded
 * @param {string} fileKey - S3 key for the file
 * @param {Object} headers - Headers to apply to the file
 * @param {Object} logger - Optional logger instance
 */
function uploadFile(aws, bucketName, filePath, fileKey, headers, logger = null) {
    let baseHeaderKeys = [
        'Cache-Control',
        'Content-Disposition',
        'Content-Encoding',
        'Content-Language',
        'Content-Type',
        'Expires',
        'Website-Redirect-Location'
    ];

    const fileBuffer = fs.readFileSync(filePath);
    const fileSize = fileBuffer.length;
    const contentType = mime.lookup(filePath);

    let params = {
        Bucket: bucketName,
        Key: fileKey,
        Body: fileBuffer,
        ContentType: contentType
    };

    Object.keys(headers).forEach(h => {
        if (baseHeaderKeys.includes(h)) {
            params[h.replace('-', '')] = headers[h];
        } else {
            if (!params.Metadata) {
                params.Metadata = {};
            }
            params.Metadata[h] = headers[h];
        }
    });

    if (logger && logger.isDebug()) {
        logger.debug(`Uploading file: ${fileKey}`, {
            size: `${(fileSize / 1024).toFixed(2)} KB`,
            contentType,
            headers: Object.keys(headers).length > 0 ? Object.keys(headers) : 'none'
        });
    }

    return aws.request('S3', 'putObject', params).then(resp => {
        if (logger && logger.isDebug()) {
            logger.logAwsResponse('S3', 'putObject', resp);
        }
        return resp;
    });
}

function buildUploadList(files, clientRoot, headerSpec, logger = null) {
    clientRoot = path.normalize(clientRoot);
    if (!clientRoot.endsWith(path.sep)) {
        clientRoot += path.sep;
    }

    if (logger) {
        logger.verbose(`Building upload list from ${files.length} file(s)`);
        if (headerSpec) {
            const specKeys = Object.keys(headerSpec);
            logger.verbose(`Applying header specifications: ${specKeys.length} rule(s)`);
        }
    }

    const uploadList = files.map(f => {
        const filePath = path.normalize(f);
        const fileRelPath = filePath.replace(clientRoot, '');
        const fileKey = path
            .normalize(fileRelPath)
            .split(path.sep)
            .join('/');

        let upload = {
            filePath: filePath,
            fileKey: fileKey,
            headers: {}
        };

        if (!headerSpec) {
            return upload;
        }

        // add bucket-wide headers
        if (headerSpec.ALL_OBJECTS) {
            headerSpec.ALL_OBJECTS.forEach(h => {
                upload.headers[h.name] = h.value;
            });
        }

        // add folder-level headers
        Object.keys(headerSpec)
            .filter(s => s.endsWith(path.sep)) // folders
            .sort((a, b) => a.length > b.length) // sort by length ascending
            .forEach(s => {
                if (fileRelPath.startsWith(path.normalize(s))) {
                    headerSpec[s].forEach(h => {
                        upload.headers[h.name] = h.value;
                    });
                }
            });

        // add file-specific headers
        Object.keys(headerSpec)
            .filter(s => {
                return path.normalize(s) === fileRelPath;
            })
            .forEach(s => {
                headerSpec[s].forEach(h => {
                    upload.headers[h.name] = h.value;
                });
            });

        return upload;
    });

    if (logger) {
        const filesWithHeaders = uploadList.filter(u => Object.keys(u.headers).length > 0).length;
        logger.verbose(`Upload list built: ${uploadList.length} file(s), ${filesWithHeaders} with custom headers`);
    }

    return uploadList;
}

module.exports = uploadDirectory;
