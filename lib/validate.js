 ...
 */
function validateClient(serverless, options, logger = null) {
    let validationErrors = [];

    if (logger) {
        logger.verbose('Starting configuration validation...');
    }
    ...
    if (logger) {
        logger.verbose(`Checking distribution folder: ${clientPath}`);
    }
    
    if (!serverless.utils.dirExistsSync(clientPath)) {
        validationErrors.push(`Could not find '${clientPath}' folder in your project root`);
        if (logger) {
            logger.debug(`Directory check failed: ${clientPath}`);
        }
    } else if (logger) {
        logger.verbose(`Distribution folder found: ${clientPath}`);
    }

    // bucketName must be a string
    if (logger) {
        logger.verbose(`Validating bucket name configuration...`);
    }
    
    if (!is.string(options.bucketName)) {
        validationErrors.push('Please specify a bucket name for the client in serverless.yml');
        if (logger) {
            logger.debug(`Bucket name validation failed: ${typeof options.bucketName}`);
        }
    } else if (logger) {
        logger.verbose(`Bucket name validated: ${options.bucketName}`);
    }

    // check header options
    if (options.objectHeaders) {
        if (logger) {
            logger.verbose(`Validating objectHeaders configuration...`);
        }
        
        if (!is.object(options.objectHeaders)) {
            validationErrors.push('objectHeaders must be an object');
            if (logger) {
                logger.debug(`objectHeaders type check failed: ${typeof options.objectHeaders}`);
            }
        } else {
            const headerKeys = Object.keys(options.objectHeaders);
            if (logger) {
                logger.verbose(`Found ${headerKeys.length} object header rule(s)`);
            }
            ...
            Object.keys(options.objectHeaders).forEach(p => {
                if (!is.array(options.objectHeaders[p])) {
                    ...
                    if (logger) {
                        logger.debug(`Header rule '${p}' is not an array`);
                    }
                }
                ...
                options.objectHeaders[p].forEach(h => {
                    if (!(is.existy(h.name) && is.string(h.name))) {
                        ...
                        if (logger) {
                            logger.debug(`Header missing name attribute in rule '${p}'`);
                        }
                    }
                    ...
                    if (!(is.existy(h.value) && is.string(h.value))) {
                        ...
                        if (logger) {
                            logger.debug(`Header missing value attribute in rule '${p}'`);
                        }
                    }
                });
            });
            
            if (logger && validationErrors.length === 0) {
                logger.verbose(`Object headers validation passed`);
            }
        }
    }
    ...
    if (validationErrors.length > 0) {
        if (logger) {
            ...
            validationErrors.forEach((error, index) => {
                logger.debug(`Validation error ${index + 1}: ${error}`);
            });
        }
