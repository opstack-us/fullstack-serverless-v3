...
class Logger {
    ...
    /**
     * Determine the current log level based on Serverless CLI options and environment
     * @returns {string} - One of: 'debug', 'verbose', 'info'
     */
    determineLogLevel() {
        // Check for SLS_DEBUG environment variable (highest priority)
        const slsDebug = process.env.SLS_DEBUG;
        if (slsDebug) {
            // If SLS_DEBUG is set to '*' or contains 'fullstack-serverless', enable debug
            if (slsDebug === '*' || slsDebug.includes('fullstack-serverless')) {
                return 'debug';
            }
            // Any other SLS_DEBUG value enables verbose
            return 'verbose';
        }
    ...
    uploadProgress(current, total, itemName = null, lastPercent = -1) {
        ...
        // Always show progress updates - more frequent for verbose mode
        // For small uploads (< 50 files), show every file
        // For larger uploads, show every 5% or every N files, whichever comes first
        const isSmallUpload = total < 50;
        const showEveryN = this.isVerbose() ? 5 : 10; // Show every 5 files in verbose, 10 otherwise
        const showEveryPercent = this.isVerbose() ? 2 : 5; // Show every 2% in verbose, 5% otherwise
        
        // Always show progress - don't require verbose mode
        const shouldShow = current === 0 || // Always show start
                         current === total || // Always show completion
                         isSmallUpload || // Always show for small uploads
                         (current % showEveryN === 0) || // Show every N files
                         (currentPercent !== lastPercent && currentPercent % showEveryPercent === 0); // Show every N%
        
        if (shouldShow) {
            const percentage = Math.round((current / total) * 100);
            const itemInfo = this.isVerbose() && itemName ? ` - ${itemName}` : '';
            // Use a more visible format for progress
            const progressBar = this._createProgressBar(current, total);
            const status = current === total ? 'Completed' : 'Uploading';
            this.serverless.cli.consoleLog(
                chalk.cyan(`  → ${status}: ${progressBar} ${current}/${total} (${percentage}%)${itemInfo}`)
            );
        }
